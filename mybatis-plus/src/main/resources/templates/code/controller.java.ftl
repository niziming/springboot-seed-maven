...
import ${package.Entity}.<#if entity?ends_with("DTO")>${entity? cap_first? substring(0, entity? index_of("DTO"))}<#else>${entity? cap_first}</#if>DTO;
import ${package.Entity?substring(0, package.Entity? index_of("dto"))}vo.<#if entity?ends_with("DTO")>${entity? cap_first? substring(0, entity? index_of("DTO"))}<#else>${entity? cap_first}</#if>VO;
...
<#--    public ResponseResult<IPage<<#if entity?ends_with("DTO")>${entity? cap_first? substring(0, entity? index_of("DTO"))}<#else>${entity? cap_first}</#if>VO>> getByPage(@Validated <#if entity?ends_with("DTO")>${entity? cap_first? substring(0, entity? index_of("DTO"))}<#else>${entity}</#if>DTO <#if entity?ends_with("DTO")>${entity? substring(0, entity? uncap_first? index_of("DTO"))}<#else>${entity? uncap_first}</#if>) {-->
    public ResponseResult<IPage<<#if entity?ends_with("DTO")>${entity? cap_first? substring(0, entity? index_of("DTO"))}<#else>${entity? cap_first}</#if>VO>> getByPage(@Validated ${entity} ${entity? uncap_first}) {
        // TODO 分页查询
    ...
    public ResponseResult add(@RequestBody @Validated ${entity} ${entity? uncap_first}) {
        // TODO 新增
    ...
    public ResponseResult update(@RequestBody @Validated ${entity} ${entity? uncap_first}) {
        // TODO 修改
